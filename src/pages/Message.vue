    ...
    <div>
        <div class="flex flex-col items-center transition duration-500 ease-in-out z-20 sticky">
            ...
            <div class="flex flex-wrap">
                <div :class="current === item.id ? ['border-1 border-gray-900', 'dark:border-gray-300', 'rounded-full', 'font-semibold'] : ['text-gray-500']"
        ...
        <div class="m-auto mt-10 grid flex-wrap pb-10 gap-5 justify-center"
            ...
            <MessageCard v-for="item in 30" :key="item" :cardInfo="{ item }" @click="showCardDetail(item)" />
        </div>
...
const chooseType = (item: number) => {
    current.value = item
}

const showCardDetail = (item: any) => {
    messageDetail.value.cardId = item
    messageDetail.value.getInfoById()
}
</script>